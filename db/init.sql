...
DROP TRIGGER IF EXISTS trg_auditoria_links ON gle_links;
...
COMMENT ON COLUMN gle_categorias.id_categoria IS 'Identificador único da categoria';
COMMENT ON COLUMN gle_categorias.ds_categoria IS 'Descrição/nome da categoria';
COMMENT ON COLUMN gle_categorias.area_tecnica IS 'Área técnica relacionada à categoria';
COMMENT ON COLUMN gle_categorias.icone IS 'Ícone representativo da categoria';
...
COMMENT ON TABLE gle_secoes IS 'Armazena as seções de agrupamento dos links';
COMMENT ON COLUMN gle_secoes.id_secao IS 'Identificador único da seção';
COMMENT ON COLUMN gle_secoes.ds_secao IS 'Descrição/nome da seção';
COMMENT ON COLUMN gle_secoes.sigla_secao IS 'Sigla da seção (ex: SP, RJ)';
...
COMMENT ON TABLE gle_links IS 'Armazena todos os links cadastrados no sistema';
COMMENT ON COLUMN gle_links.id_link IS 'Identificador único do link';
COMMENT ON COLUMN gle_links.link IS 'URL do link';
COMMENT ON COLUMN gle_links.ds_link IS 'Descrição do link';
COMMENT ON COLUMN gle_links.titulo IS 'Título descritivo do link';
COMMENT ON COLUMN gle_links.id_categoria IS 'Referência à categoria (FK)';
COMMENT ON COLUMN gle_links.id_secao IS 'Referência à seção (FK)';
COMMENT ON COLUMN gle_links.area_tecnica IS 'Área técnica relacionada';
COMMENT ON COLUMN gle_links.ativo IS 'Indica se o link está ativo';
...
COMMENT ON TABLE gle_auditoria IS 'Registra todas as operações realizadas nos links para rastreabilidade';
COMMENT ON COLUMN gle_auditoria.id_auditoria IS 'Identificador único do registro de auditoria';
COMMENT ON COLUMN gle_auditoria.id_link IS 'Referência ao link auditado (FK)';
COMMENT ON COLUMN gle_auditoria.acao IS 'Tipo de ação realizada: INSERT, UPDATE ou DELETE';
COMMENT ON COLUMN gle_auditoria.conteudo_antes IS 'Conteúdo do registro antes da alteração (JSON)';
COMMENT ON COLUMN gle_auditoria.conteudo_depois IS 'Conteúdo do registro após a alteração (JSON)';
COMMENT ON COLUMN gle_auditoria.dt_auditoria IS 'Data e hora da ação';


-- 5 Tabela: gle_areas_tecnicas
CREATE TABLE IF NOT EXISTS gle_areas_tecnicas (
...
-- Seed: areas_tecnicas
INSERT INTO gle_areas_tecnicas (nome_area) VALUES
    ('Todas'),
    ('Tributário'),
    ('Suprimentos'),
    ('Rh/Administrativo'),
...
CREATE INDEX IF NOT EXISTS idx_links_categoria    ON gle_links(id_categoria);
CREATE INDEX IF NOT EXISTS idx_links_secao        ON gle_links(id_secao);
CREATE INDEX IF NOT EXISTS idx_links_area_tecnica ON gle_links(area_tecnica);
CREATE INDEX IF NOT EXISTS idx_links_ativo        ON gle_links(ativo);
CREATE INDEX IF NOT EXISTS idx_auditoria_link     ON gle_auditoria(id_link);
CREATE INDEX IF NOT EXISTS idx_auditoria_acao     ON gle_auditoria(acao);
CREATE INDEX IF NOT EXISTS idx_auditoria_data     ON gle_auditoria(dt_auditoria);
...
BEGIN
    IF (TG_OP = 'DELETE') THEN
        INSERT INTO gle_auditoria (id_link, acao, conteudo_antes, conteudo_depois)
        VALUES (OLD.id_link, 'DELETE', row_to_json(OLD)::TEXT, NULL);
        RETURN OLD;
    ELSIF (TG_OP = 'UPDATE') THEN
        INSERT INTO gle_auditoria (id_link, acao, conteudo_antes, conteudo_depois)
        VALUES (NEW.id_link, 'UPDATE', row_to_json(OLD)::TEXT, row_to_json(NEW)::TEXT);
        RETURN NEW;
    ELSIF (TG_OP = 'INSERT') THEN
        INSERT INTO gle_auditoria (id_link, acao, conteudo_antes, conteudo_depois)
        VALUES (NEW.id_link, 'INSERT', NULL, row_to_json(NEW)::TEXT);
        RETURN NEW;
    END IF;
    RETURN NULL;
END;
